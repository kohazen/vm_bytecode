/*
 * bytecode_loader.c - Bytecode File Loading Implementation
 *
 * Loads bytecode files into the VM with format validation.
 */
...
static bool read_uint32(FILE *file, uint32_t *value) {
    ...
    /* Reconstruct 32-bit value from little-endian bytes */
...
VMError vm_load_bytecode_file(VM *vm, const char *filename) {
    ...
    /* Free any previously loaded code */
    vm_free_bytecode(vm);
