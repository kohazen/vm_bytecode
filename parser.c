 ...
 *
 * Parses tokens into instructions using the opcode table.
 */
...
bool parser_parse(Parser *parser) {
    while (!is_at_end(parser)) {
        ...
        /* Skip label definitions (we'll handle them in Day 3) */
        ...
        if (!entry) {
            /* Not a known instruction - might be a label reference */
            snprintf(parser->error_msg, sizeof(parser->error_msg),
        ...
        /* If instruction has an operand, read it */
        if (entry->has_operand) {
            ...
            if (operand->type == TOKEN_NUMBER) {
                ...
                /* This is actually a label reference, not an instruction */
